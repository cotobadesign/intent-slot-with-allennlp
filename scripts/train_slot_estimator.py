...
from slot_estimator import SlotEstimator, SlotEstimatorPredictor
...
if __name__ == '__main__':
    ...
    model = SlotEstimator(vocab, embedder, encoder)
    model.cuda()
    ...
    predictor = SlotEstimatorPredictor(model, dataset_reader)

    text = dataset_reader.tokenizer('東京駅から富山駅まで行きたいです')
    output = predictor.predict(text)
    for id, mrph in zip(np.argmax(np.asarray(output['probs']), axis=1), text):
